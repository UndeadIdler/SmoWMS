        ...
        public ReturnInfo AddConInventoryResult(AddCIResultInputDto inputDto)
        {
            ...
            else
            {
                try
                {
                    ...
                    {
                        //添加当前的盘点单行项
                        List<string> conList = _conQuantRepository.GetInventoryCons(inventory.WAREID, inventory.STID, inventory.SLID).Select(a => a.CID).ToList();
                        ...
                        {
                            ConQuant conQuant = _conQuantRepository.GetQuants(inventory.WAREID, con).FirstOrDefault();
                            ConInventoryResult result = new ConInventoryResult
                            {
                                IID = inputDto.IID,
                                CID = con,
                            ...
                            //更新盘点的物品状态为锁定
                            var conq = _conQuantRepository.GetQuants(inputDto.WAREID, con).FirstOrDefault();
                            if (conq != null)
                            {
                                conq.ISLOCKED = 1;
